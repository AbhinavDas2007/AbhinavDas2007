# Hi, I'm Abhinav Pradeep Das 👋

### Aspiring Data Scientist | Data Analytics | Python | SQL | Power BI

I'm an engineering student at NIT Raipur interested in **Data Science, Data Analytics, and Business Intelligence**.

I enjoy working with data to uncover patterns, generate insights, and build meaningful visualizations and analytical solutions.

---

## 🛠️ Skills & Tools

**Programming & Data Analysis**
- 🐍 Python
- 🔢 NumPy
- 🗄️ SQL
- 🐬 MySQL

**Data Visualization & BI**
- 📊 Power BI
- 📗 Microsoft Excel
- 📈 Data Visualization

**Web Technologies**
- 🌐 HTML
- 🎨 CSS
- ⚡ JavaScript

**Tools**
- 🔧 Git
- 🐙 GitHub
- 💻 VS Code

---

## 📊 Featured Project

### 🎓 Student Performance Data Analysis

A complete data analytics project analyzing student academic performance.

**Technologies used:**
`Excel` • `Python` • `NumPy` • `MySQL` • `SQL` • `Power BI` • `HTML` • `CSS` • `JavaScript`

The project covers:

- Data preparation and cleaning
- Numerical analysis using Python & NumPy
- SQL-based data analysis using MySQL
- Interactive Power BI dashboard
- Web-based project presentation

🔗 **[View Project](YOUR_PROJECT_REPOSITORY_LINK)**

---

## 🎯 Currently Learning

- 📊 Data Analytics
- 🐍 Python for Data Science
- 🗄️ Advanced SQL
- 📈 Power BI & Business Intelligence
- 🤖 Machine Learning

---

## 🔗 Connect With Me

💼 **LinkedIn:** (www.linkedin.com/in/abhinav-d-661525386)

🐙 **GitHub:** (https://github.com/AbhinavDas2007)

🌐 **Portfolio:** [My Portfolio](YOUR_PORTFOLIO_LINK)

---

### 💡 "Turning data into insights, one analysis at a time." 📊
